...
predictor = dlib.shape_predictor('visual_detect/weights/shape_predictor_68_face_landmarks.dat')
# 分别获取左右眼面部标志的索引
...
def detfatigue(frame):
    ...
    for rect in rects:
        ...
        # 画出眼睛、嘴巴竖直线
        cv2.line(frame,tuple(shape[38]),tuple(shape[40]),(0, 255, 0), 2)
        cv2.line(frame,tuple(shape[43]),tuple(shape[47]),(0, 255, 0), 2)
        cv2.line(frame,tuple(shape[51]),tuple(shape[57]),(0, 255, 0), 2)
        cv2.line(frame,tuple(shape[48]),tuple(shape[54]),(0, 255, 0), 2)
